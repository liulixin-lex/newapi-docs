...
![Windows 运行窗口中输入 .codex 配置目录路径](/images/tutorial/manual-codex-01-windows-run-path.webp)

1. 你的目录中可能会存在以下文件，不过我们用到的文件只有三个，需要配置的只有两个

![Windows 的 Codex 配置目录，显示 config.toml、auth.json 和 AGENTS.md](/images/tutorial/manual-codex-02-windows-config-dir.webp)
...
![Windows auth.json 中填写 Codex 使用的 OPENAI_API_KEY](/images/tutorial/manual-codex-03-auth-json-key.webp)

回顾 [创建API令牌](/getting-started/api-token.html) 这一步教程，我们需要在GGUU API 中创建 Codex 分组的令牌，然后点击右侧的复制按钮，将 Key 填入 `xxx` 部分后保存

![GGUU API 控制台中复制 Codex 分组令牌](/images/tutorial/shared-codex-token-console.webp)
...
![Windows 终端运行 Codex 并测试回复](/images/tutorial/shared-codex-terminal-chat.webp)
...
![macOS 访达前往文件夹窗口中输入 .codex 配置目录](/images/tutorial/manual-codex-04-macos-folder-path.webp)

1. 你的目录中可能会存在以下文件，不过我们用到的文件只有三个，需要配置的只有两个，其它文件可忽略

![macOS 的 Codex 配置目录，显示 config.toml、auth.json 和 AGENTS.md](/images/tutorial/manual-codex-05-macos-config-dir.webp)
...
![macOS auth.json 中填写 Codex 使用的 OPENAI_API_KEY](/images/tutorial/shared-codex-token-console.webp)

回顾 [创建API令牌](/getting-started/api-token.html) 这一步教程，我们需要在GGUU API中创建 Codex 分组的令牌，然后点击右侧的复制按钮，将 Key 填入 `xxx` 部分后保存

![GGUU API 控制台中复制 Codex 分组令牌](/images/tutorial/manual-codex-03-auth-json-key.webp)
...
![macOS 终端运行 Codex 并测试回复](/images/tutorial/shared-codex-terminal-chat.webp)
