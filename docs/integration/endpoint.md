...
![数据看板左下角显示主站和优化线路 API 请求地址](/images/tutorial/api-endpoint-01-api-addresses.webp)

主站 Endpoint：`{{API_BASE_URL}}`，推荐优先使用，适合对稳定性要求较高的日常场景。

优化线路 Endpoint：`{{OPTIMIZED_API_BASE_URL}}`，优化线路，适合对延迟敏感的场景。
