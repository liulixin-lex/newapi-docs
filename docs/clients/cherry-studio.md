...
![Cherry Studio 设置中的模型服务页面](/images/tutorial/80.png)

1. 在模型服务列表底部点击 添加，新增一个提供商，并参考图中填写。

![Cherry Studio 模型服务列表底部添加新提供商](/images/tutorial/81.png)
...
![Cherry Studio 的 GGUUAI 提供商 API 密钥和 API 地址配置](/images/tutorial/82.png)

1. 点击 `获取模型列表`，在模型窗口中选择你要使用的模型，例如 `gpt-5.5`

![Cherry Studio 获取模型列表并选择 gpt-5.5 模型](/images/tutorial/83.png)

1. 点击右上角 `检测` 按钮，提示 `连接成功` 即可使用。

![Cherry Studio 检测连接成功提示](/images/tutorial/84.png)

1. 回到 `首页`，左上角选择模型，进行聊天。

![Cherry Studio 首页中选择模型并开始聊天](/images/tutorial/85.png)
